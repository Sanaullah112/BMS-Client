...
export default function DepositPage() {
  ...
  const onConfirm = async () => {
    setSubmitting(true);
    try {
      const response = await deposit({ accountId, amount: Number(amount), description });
      await refresh();
      setResult({
        type: 'Deposit',
      ...
      push(err.friendlyMessage || 'Deposit failed.', 'error');
      setConfirmOpen(false);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted">Credit funds into a customer's account.</p>
      </div>
      ...
      <Card className="max-w-lg">
        <form onSubmit={handleSubmit(() => setConfirmOpen(true))} className="flex flex-col gap-4" noValidate>
          ...
          <Button type="submit" variant="success" icon={ArrowDownToLine} disabled={!accountId || account?.status !== 'Active'}>
            Review deposit
          </Button>
          {account && account.status !== 'Active' && (
            <p className="text-xs text-danger">This account is {account.status.toLowerCase()} and cannot receive deposits.</p>
          )}
        </form>
      </Card>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="success" onClick={onConfirm} loading={submitting}>Confirm deposit</Button>
          </>
        }
      >
        <div className="flex flex-col gap-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-text-muted">New balance</span><span className="font-medium tnum">{formatCurrency((account?.balance || 0) + (Number(amount) || 0))}</span></div>
        </div>
      </Modal>
